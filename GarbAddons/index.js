...
registerWhen(register("chat", (player, command, event) => {
  ...
  delay(() => {
    if (command.includes("raider")) {
      ChatLib.say(`/party chat Why do Kuudra Players feel the need to rs runs because of their poor problem solving skills??? It doesnt give you better runs and only gets you less runs and consequently money per hour...`);
    }
    else if (command.includes("Lovekuudra")) {
      ChatLib.say(`/party chat Supplies: 98 | Build: 89 | Stun: 0.4 | Kuudra 94 https://i.imgur.com/EBsp38Z.png`)
    }
      ...
      ChatLib.say(`/party chat Supplies: 90 | Build: N/A (LEAVES EARLY) | Stun: Plancks Constant | Kuudra: 91`)
    }
    else if (command.includes("PB")) {
      ChatLib.say(`/party chat ${Settings.PB}`)
    }
...
// Commands
register("command", () => {
  ChatLib.chat(`&r&d&m--------------&rCOMMANDS&r&d&m--------------`)
  ChatLib.chat(`&e /pc .INSERT PLAYER NAME -- Get Player Stats `)
  ChatLib.chat(`&e /pc .PB -- See everyones pb!`)
  ChatLib.chat(`&e /pc .ANY ELEMENT 1 H -- 12 Mg `)
  ChatLib.chat(`&r&d&m--------------&rCOMMANDS&r&d&m--------------`)
}).setName('garb list', true);
